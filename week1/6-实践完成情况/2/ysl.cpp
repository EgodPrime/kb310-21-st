...
Graph::Graph(bool directed){ //graph的构造函数
...
void Graph::insert_edge(int x, int y, int weight, bool directed){ //构造边
...
int main(){
    ...
    Graph *g = new Graph(false); //new一个Graph类
    int parent[MAXV + 1];
    int distance[MAXV + 1];
    int start = 1;
    ...
    g->insert_edge(4, 5, 2, false); //构建图

    g->print();
    
    dijkstra_shortest_path(g, parent, distance, start); //调用函数寻找最短路径
    print_distances(start, distance); //打印从顶点 1 到 5 的最短路径
